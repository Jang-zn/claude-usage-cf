...
- **Quota 현황** — Anthropic API로 실제 한도값을 역산한 뒤, 이후엔 로컬 토큰 수치로 계산 (30분 주기 + 리셋 시간 도달 시 자동 재동기화)
...
- Anthropic API (`/api/oauth/usage`) 호출은 **최소화** — 한도값 역산 후 로컬 계산, 30분 주기 또는 리셋 시간 도달 시에만 재호출
